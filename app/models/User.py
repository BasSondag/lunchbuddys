...
class User(Model):
    ...
    def add_to_aplist(self,info):     
        create_appointmet_list_query = "INSERT INTO appointments (linkedin_users_id, created_at, updated_at) VALUES (:linkedin_users_id, NOW(), NOW())"
        appoint_list_data = {'linkedin_users_id': info['user_id']}
        return self.db.query_db(create_appointmet_list_query, appoint_list_data)

    def show_applist(self):
        show_query = "SELECT linkedin_users.headline, linkedin_users.industry, linkedin_users.location, appointments.linkedin_users_id FROM linkedin_users JOIN appointments ON linkedin_users.id = appointments.linkedin_users_id"
